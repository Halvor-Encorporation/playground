...
export const WheelComponent = ({
  ...
  buttonText = "Spin",
  isOnlyOnce = false,
  size = window.innerWidth < 600 ? window.innerWidth / 2 : window.innerWidth / 4,
  upDuration = Math.floor(Math.random() * (250 - 100 + 1)) + 100,
  downDuration = Math.floor(Math.random() * (500 - 300 + 1)) + 300,
  ...
  return (
    <div id={wheelId.current}>
      <canvas
        ...
        height={dimension}
        style={{
          pointerEvents: (isFinished && isOnlyOnce) || disabled ? "none" : "auto",
      }}
      />
    </div>
  );
